 ...
 * If the TCP peer is Cloudflare or loopback → CF-Connecting-IP (then X-Forwarded-For).
...
/**
 * Express `trust proxy` callback: trust Cloudflare edges and local reverse proxies.
 * Without loopback, Apache→Node on 127.0.0.1 ignores X-Forwarded-Proto and
 * secure session cookies never stick (OAuth login loop).
 */
function expressTrustProxy(addr) {
  const n = normalizeIp(addr);
  return isLocalhost(n) || isCloudflareIp(n);
}
